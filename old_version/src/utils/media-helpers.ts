import { MessageAttachment, attachmentSchema } from '../types/index.js';
...
export function detectMediaInContent(content: string): MessageAttachment[] {
  ...
  // Detect documents
  const docRegex = /https?:\/\/\S+\.(pdf|doc|docx|xls|xlsx|ppt|pptx|txt)(\?[^"']*)?(?=['")\s]|$)/gi;
  ...
  if (docMatches.length > 0) {
    ...
    docUrls.forEach((docUrl, index) => {
      ...
      else if (fileType === 'doc' || fileType === 'docx') mimeType = 'application/msword';
      else if (fileType === 'xls' || fileType === 'xlsx') mimeType = 'application/vnd.ms-excel';
      else if (fileType === 'ppt' || fileType === 'pptx') mimeType = 'application/vnd.ms-powerpoint';
      ...
      if (validation.success) {
        ...
        console.warn('Document attachment validation failed:', validation.error);
        attachments.push(docItem);
      }
    });
  }
  ...
  // Detect other files
  const fileRegex = /https?:\/\/\S+\.(zip|rar|tar|gz|exe|dmg|bin)(\?[^"']*)?(?=['")\s]|$)/gi;
  ...
  if (fileMatches.length > 0) {
    ...
    fileUrls.forEach((fileUrl, index) => {
      ...
      else if (fileType === 'tar' || fileType === 'gz') mimeType = 'application/x-compressed';
  ...
  const linkRegex = /https?:\/\/(?!.*\.(jpg|jpeg|png|gif|webp|mp3|wav|ogg|m4a|mp4|webm|mov|avi|pdf|doc|docx|xls|xlsx|ppt|pptx|txt|zip|rar|tar|gz|exe|dmg|bin))[^\s]+/gi;
 ...
 */
export function detectDataRequestInContent(content: string): string | undefined {
  if (typeof content !== 'string' || !content.trim()) {
    return undefined;
  }
...
export function processMessageContent(content: string): {
  ...
  if (dataRequest) {
    ...
    if (schemaIndex !== -1) {
      processedContent = processedContent.substring(0, schemaIndex).trim() + 
      ...
      processedContent = 'This message contains a form schema. Click the button below to provide the requested information.';
    }
  }
